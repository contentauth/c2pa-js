...
describe('ingredient', () => {
  ...
  test('builder.addIngredient should accept an IngredientRef and relationship', async ({
    c2pa,
  }) => {
    const blob = await getBlobForAsset(C_with_CAWG_data);
    ...
    await builder.addIngredient(ingredientRef, 'componentOf');
  ...
  test('builder.addIngredient should work with a rehydrated IngredientRef', async ({
    c2pa,
  }) => {
    const blob = await getBlobForAsset(C_with_CAWG_data);
    ...
    const builder = await c2pa.builder.new();
    await builder.addIngredient(rehydratedIngredientRef, 'componentOf');
